	...
	protected void findMacros() {
		...
		for (String line : this.code.split("\n")) {
			if (line.contains("//! textmacro")) {
				...
				this.macros.put(name,
						new Macro(name, params, String.join("\n", body)));
		...
		for (String line : this.code.split("\n")) {
			if (line.contains("//! textmacro")) {
				...
				if (line.contains("//! runtextmacro")) {
					...
					args = new PriorityQueue<String>(
							this.getArgumentsFromText(line));

					if (args.size() != macro.getParams().size()) {
						throw new IncorrectArgumentsTextmacroException(
								macroName);
					}
